package OpenILS::WWW::MLCExporter;
use strict;
use warnings;
use bytes;
...
#								if ( ($orgs{$cp->circ_lib}->shortname =~ /^BDL/) 
#									&& ($cp->status != 9)
#									&& ($cp->circ_modifier !~ /COMPANY|ILL-RET|MISCELLANEOUS|IPOD/) ){


                                                                if ( ($cp->status != 9)                   # don't include ON ORDER
                                                                        && ($cp->circ_modifier !~ /OTHER|LAPTOP|DCB/)
                                                                        && ($cp->circ_modifier !~ /21DGEN|7DMAG/)				 #GRPL
                                                                ){
                                                                        my $circ_mod=$cp->circ_modifier;

									if ( (($cp->circ_modifier =~ /21D|7D/) && ($shelves{$cp->location}->name eq "New")) 
										|| ($shelves{$cp->location}->name eq "Express") 
										|| ($cp->circ_modifier eq "42D")
										|| ($cn->label =~ /booklet|libretto|program notes|lyric|^Oversize|SuDoc/)
										|| (($cp->circ_lib == 10) && ($cn->label =~ /^USB|^MP3|^Playaway|^Furn\.\sColl\./)) ) {
										$circ_mod = "DNC";	
									}
