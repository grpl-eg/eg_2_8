...
sub import {
	...
	for my $pkg ( __PACKAGE__->classes ) {
		(my $cdbi = $pkg) =~ s/^OnlinePay::Fieldmapper:://o;

		eval <<"		PERL";
			package $pkg;
			use base 'OnlinePay::Fieldmapper';
		PERL
		...
		OnlinePay::JSON->register_class_hint(
			hint => $pkg->json_hint,
			name => $pkg,
			type => 'array',
		);
